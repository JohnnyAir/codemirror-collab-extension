...
import {
  PeerSelectionState,
  peerSelectionField,
  peerSelectionsAnnotation,
} from './peer-selection-state';
...
class PeerSelectionPlugin {
  ...
  _computeSelectionsDecorations(update: ViewUpdate) {
    const decorationsRange = this.peerSelectionState.selectionsRanges
      .map((peerRange) => {
        if (
          peerRange.range.empty ||
          peerRange.range.from === peerRange.range.to
        ) {
  ...
  _getSelectionRangeDecoration(
    ...
    if (isSingleLineSelection) {
      ...
      if (fromLine.length !== 0) {
        decorations.push(
          Decoration.mark(decorationOption).range(from, from + fromLine.length)
        );
      }
